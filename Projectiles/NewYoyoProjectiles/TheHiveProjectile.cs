			...
			if (OnHitEffectCooldown == 0)
			{
				for (int num591 = 0; num591 < 3; num591++)
				{
					if (num591 == 1 || Main.rand.Next(2) == 0)
					{
						...
						int num594 = Projectile.NewProjectile(Projectile.GetSource_FromThis(), position8.X, position8.Y, num592, num593, Main.player[Projectile.owner].beeType(), Main.player[Projectile.owner].beeDamage(target.damage / 3), Main.player[Projectile.owner].beeKB(0f), Main.myPlayer);
